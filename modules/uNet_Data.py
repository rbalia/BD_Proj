...
def create_train_data(cropOnMask=False):
    ...
    for fileNameFull in os.listdir(conf.setDir + conf.trnDir):
        ...
        if conf.channels == 3:
            ...
            img_mask = imread(os.path.join(conf.setDir + conf.trnDir + conf.mskDir, fileName + ".png"))
        else:
            img = imread(os.path.join(conf.setDir + conf.trnDir, fileName+"." + fileExtension), as_gray=True)
            img_mask = imread(os.path.join(conf.setDir + conf.trnDir + conf.mskDir, fileName+".png"), as_gray=True)

        """###
        nonZeroPixels = cv.findNonZero(img_mask)
        x, y, w, h = cv.boundingRect(nonZeroPixels)
        pad = 10
        max = np.max([w, h])
        x = x - int(abs(w - max) / 2)
        y = y - int(abs(h - max) / 2)
        img = img[y - pad:y + max + pad, x - pad:x + max + pad]
        img_mask = img_mask[y - pad:y + max + pad, x - pad:x + max + pad]
        img_mask[img_mask > 0] = np.max(img_mask)
        ###
        """
...
def create_test_data():
    ...
    for fileName in os.listdir(data_path):
        ...
        if conf.channels == 3:
            ...
            img_mask = imread(os.path.join(data_path + conf.mskDir, fileName + ".png"))
        else:
            img = imread(os.path.join(data_path, fileName+".jpg"), as_gray=True)
            img_mask = imread(os.path.join(data_path + conf.mskDir, fileName + ".png"), as_gray=True)
